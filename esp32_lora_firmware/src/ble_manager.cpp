...
class RxCallbacks : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic* c, NimBLEConnInfo& info) override {
    ...
    // รับไบนารีตรงจากแอป: ResilNetRadioPacket packed
...
void BleGatewayManager::setupServer() {
  ...
  info->setValue("{\"role\":\"gateway\",\"proto\":1}");
...
void BleGatewayManager::taskBle(void* arg) {
  (void)arg;
  Serial.println("[BLE] TaskBLE idle monitor");
  while (true) {
...
void BleGatewayManager::startTask() {
  xTaskCreatePinnedToCore(taskBle, "TaskBLE", TASK_BLE_STACK, this, TASK_BLE_PRIO, nullptr, 0);
}
